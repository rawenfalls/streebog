...
void init(struct stribog_ctx_t *ctx, u8 size)//инициализация
{
	memset(ctx->N, 0x00, BLOCK_SIZE);//устанавливает все ctx->N в 0x00
	memset(ctx->S, 0x00, BLOCK_SIZE);//устанавливает все ctx->S в 0x00

	ctx->size = !!size;//двойное отрицание для явного приведения к TRUE или FALSE

	if (ctx->size)//выбираем какой блок памяти копировать
